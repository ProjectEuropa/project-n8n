...
├── setup_auth.sh         # Basic認証セットアップスクリプト
├── n8n-backup.sh         # バックアップスクリプト
├── n8n-restore.sh        # リストアスクリプト
├── caddy_config/
...
スクリプト実行後は、手順4（DNSレコード設定）と手順5（ファイアウォール設定）に進んでください。
...
提供されているバックアップスクリプトを使用すると、簡単にバックアップを実行できます。

```bash
# 1. スクリプトを編集してN8N_DIRを設定
nano n8n-backup.sh
# N8N_DIR=/path/to/your/n8n-docker-caddy の行を実際のパスに変更
...
バックアップスクリプトを使用してcronで定期的にバックアップを実行する場合：

```bash
# 1. スクリプトのN8N_DIRを設定（上記参照）
nano n8n-backup.sh

# 2. cronに登録（毎日3時に実行）
(crontab -l 2>/dev/null; echo "0 3 * * * $(pwd)/n8n-backup.sh >> $HOME/n8n-backup.log 2>&1") | crontab -
```

### 自動バックアップ（cron - 手動スクリプト作成）

バックアップスクリプトを手動で作成する場合：
...
提供されているリストアスクリプトを使用すると、簡単にバックアップから復元できます。
