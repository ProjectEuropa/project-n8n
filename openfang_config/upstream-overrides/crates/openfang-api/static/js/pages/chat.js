  ...
  return {
        ...
        if (modelMatch) {
          ...
          if (!self.modelPickerList.length) {
            Promise.all([
              ...
              self.modelPickerList = mergeImplicitDefaultModel(results[1].models || [], results[0]).filter(function(m) { return m.available; });
    ...
    toggleModelSwitcher() {
      ...
      Promise.all([
        ...
        var models = mergeImplicitDefaultModel(results[1].models || [], results[0]).filter(function(m) { return m.available; });
    ...
    switchModel(model) {
      ...
      OpenFangAPI.get('/api/status').catch(function() { return {}; }).then(function(status) {
        var selection = resolveModelSelection(model, status);
        return OpenFangAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: selection });
      }).then(function(resp) {
      ...
      switch (cmd) {
        ...
        case '/model':
          if (self.currentAgent) {
            if (cmdArgs) {
              OpenFangAPI.get('/api/status').catch(function() { return {}; }).then(function(status) {
                var selection = resolveModelSelection({ id: cmdArgs }, status);
                return OpenFangAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: selection });
              }).then(function(resp) {
    ...
    selectAgent(agent) {
      this.currentAgent = normalizeAgentModel(agent);
      this.messages = [];
